    ...
    void Update()
    {
        ...
        for (int i = 0; i < lineSegmentPositions.Length; i++)
        {
            ...
            else
            {
                ...
                lineSegmentPositions[i] = Vector3.SmoothDamp(lineSegmentPositions[i], lineSegmentPositions[i - 1] + (facingDirection * offset) + facingDirection * Random.Range(-noise, noise), ref lineSegmentVelocities[i], lagTime);
            }
